...
class ChatHeads(context: Context) : View.OnTouchListener, FrameLayout(context) {
    companion object {
        ...
        val CHAT_HEAD_EXPANDED_MARGIN_TOP: Float = dpToPx(24f).toFloat()
        val CLOSE_SIZE = dpToPx(64f)
        val CLOSE_CAPTURE_DISTANCE = dpToPx(50f)
    ...
    fun rearrangeExpanded(animation: Boolean = true) {
        ...
        chatHeads.forEachIndexed { index, it ->
            ...
            val y = metrics.heightPixels - topChatHead!!.params.height.toDouble()- CHAT_HEAD_EXPANDED_MARGIN_TOP
            if (animation) {
    ...
    fun onChatHeadSpringUpdate(chatHead: ChatHead, spring: Spring, totalVelocity: Int) {
        ...
        if (tmpChatHead != null) {
            ...
            Log.i("check@bubble","x & y")

            content.x =  0f
            content.y = topChatHead!!.y- topChatHead!!.params.height- CHAT_HEAD_EXPANDED_MARGIN_TOP
    ...
    override fun onTouch(v: View?, event: MotionEvent?): Boolean {
        ...
        when (event!!.action) {
            ...
            MotionEvent.ACTION_UP -> {
                Log.i("check@bubble","touch up button 2")
                ...
                if (!moving) {
                    if (!toggled) {
                        ...
                        rearrangeExpanded()
                        Log.i("check@bubble","touch up button 3")
